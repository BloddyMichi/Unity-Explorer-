...
public static class UniversalUI
{
    ...
    internal static void Update()
    {
        ...
        ValueChangedRelay.UpdateInstances();
    ...
    private static void LoadBundle()
    {
        ...
        if (ShouldUseBuiltinUiResources())
        {
            Universe.Log($"Unity {Application.unityVersion} detected. Skipping embedded UniverseLib UI AssetBundle load and using built-in UI resources instead.");
            SetBuiltinUiResources();
            return;
        }
    ...
    static bool ShouldUseBuiltinUiResources()
    {
        return Application.unityVersion.StartsWith("6000.", StringComparison.Ordinal);
    }

    static void SetBuiltinUiResources()
    {
        Font fallbackFont = Resources.GetBuiltinResource<Font>("Arial.ttf");
        DefaultFont = fallbackFont;
        ConsoleFont = fallbackFont;
        BackupShader = Graphic.defaultGraphicMaterial != null ? Graphic.defaultGraphicMaterial.shader : null;
    }
