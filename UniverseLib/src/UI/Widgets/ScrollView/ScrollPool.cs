...
namespace UniverseLib.UI.Widgets.ScrollView
{
    ...
    public class ScrollPool<T> : UIBehaviourModel, IEnumerable<CellInfo> where T : ICell
    {
        ...
        public override void Update()
        {
            ...
            if (!writingLocked)
            {
                ...
                if (viewChange || Content.rect.height != prevContentHeight)
                {
                    ...
                    {
                        Universe.LogWarning($"ScrollPool.Update failed safely: {e}");
                        writingLocked = false;
                        return;
                    }
        ...
        public void Initialize(ICellPoolDataSource<T> dataSource, Action onHeightChangedListener = null)
        {
            ...
            UIFactory.SetSliderValueChangedListener(slider.gameObject, slider, OnSliderValueChanged);
        ...
        private IEnumerator InitCoroutine(Action onHeightChangedListener)
        {
            ...
            UIFactory.SetScrollRectValueChangedListener(ScrollRect.gameObject, ScrollRect, OnValueChangedListener);
        ...
        private void RefreshCells(bool andReloadFromDataSource, bool setSlider)
        {
            ...
            if (jumpToBottom && HasValidPoolState() && TryGetPoolRect(bottomPoolIndex, out RectTransform bottomRect))
            {
                float diff = Viewport.MaxY() - bottomRect.MaxY();
        ...
        private void RefreshCellHeightsFast()
        {
            ...
            foreach (CellInfo cellInfo in this)
            {
                if (cellInfo.cellIndex < 0 || cellInfo.cellIndex >= CellPool.Count)
                    continue;
                if (cellInfo.dataIndex < 0 || cellInfo.dataIndex >= HeightCache.Count)
                    continue;

                try
                {
                    var rect = CellPool[cellInfo.cellIndex]?.Rect;
                    if (rect != null)
                        HeightCache.SetIndex(cellInfo.dataIndex, rect.rect.height);
                }
                catch
                {
                }
        ...
        private bool HasValidPoolState()
        {
            return CellPool != null
                && CellPool.Count > 0
                && topPoolIndex >= 0
                && bottomPoolIndex >= 0
                && topPoolIndex < CellPool.Count
                && bottomPoolIndex < CellPool.Count;
        }

        private bool TryGetPoolCell(int index, out T cell)
        {
            cell = default;
            if (CellPool == null || index < 0 || index >= CellPool.Count)
                return false;

            try
            {
                cell = CellPool[index];
                return cell != null && cell.Rect != null;
            }
            catch
            {
                return false;
        ...
        private bool TryGetPoolRect(int index, out RectTransform rect)
        {
            rect = null;
            if (!TryGetPoolCell(index, out T cell))
                return false;
        ...
        private bool ShouldRecycleTopSafe()
        {
            if (!HasValidPoolState())
                return false;

            if (!TryGetPoolRect(topPoolIndex, out RectTransform topRect))
                return false;

            if (!TryGetPoolRect(bottomPoolIndex, out RectTransform bottomRect))
                return false;

            return GetCellExtent(topRect) > RecycleViewBounds.x
                && GetCellExtent(bottomRect) > RecycleViewBounds.y;
        }

        private bool ShouldRecycleBottomSafe()
        {
            if (!HasValidPoolState())
                return false;
            ...
            return bottomRect.position.y < RecycleViewBounds.y
                && topRect.position.y < RecycleViewBounds.x;
        }
        ...
        private float GetCellExtent(RectTransform cell) => cell == null ? 0f : cell.MaxY() - contentLayout.spacing;

        private float RecycleTopToBottom()
        {
            float recycledheight = 0f;

            while (ShouldRecycleTopSafe() && CurrentDataCount < DataSource.ItemCount)
            {
                WritingLocked = true;
                if (!TryGetPoolCell(topPoolIndex, out T cell))
                    break;
        ...
        private float RecycleBottomToTop()
        {
            ...
            while (ShouldRecycleBottomSafe() && CurrentDataCount > CellPool.Count)
            {
                WritingLocked = true;
                if (!TryGetPoolCell(bottomPoolIndex, out T cell))
                    break;
