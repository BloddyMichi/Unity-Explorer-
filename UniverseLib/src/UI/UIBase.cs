...
namespace UniverseLib.UI
{
    ...
    public class UIBase
    {
        ...
        public void SetOnTop()
        {
            ...
            int childCount = UniversalUI.CanvasRoot && UniversalUI.CanvasRoot.transform
                ? UniversalUI.CanvasRoot.transform.childCount
                : 0;

            foreach (UIBase ui in UniversalUI.uiBases)
            {
                if (ui == null || !ui.Canvas || !TryGetSiblingIndex(ui, out int siblingIndex))
                    continue;

                int offset = Math.Max(0, childCount - siblingIndex);
                ui.Canvas.sortingOrder = TOP_SORTORDER - offset;
            }

            // Sort UniversalUI dictionary so update order is correct
            UniversalUI.uiBases.Sort((a, b) => GetSiblingIndexOrDefault(b).CompareTo(GetSiblingIndexOrDefault(a)));
        }

        private static int GetSiblingIndexOrDefault(UIBase ui)
        {
            return TryGetSiblingIndex(ui, out int siblingIndex)
                ? siblingIndex
                : int.MinValue;
        }

        private static bool TryGetSiblingIndex(UIBase ui, out int siblingIndex)
        {
            siblingIndex = -1;

            if (ui == null || !ui.RootObject)
                return false;

            Transform transform = ui.RootObject.transform;
