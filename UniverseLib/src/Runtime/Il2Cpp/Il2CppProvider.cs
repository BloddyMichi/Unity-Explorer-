...
namespace UniverseLib.Runtime.Il2Cpp
{
    internal class Il2CppProvider : RuntimeHelper
    {
        ...
        {
            int sceneHandle = GetSceneHandleAsInt(scene);
        ...
        /// <inheritdoc/>
        protected internal override int Internal_GetRootCount(Scene scene) => GetRootCount(GetSceneHandleAsInt(scene));

        private static int GetSceneHandleAsInt(Scene scene)
        {
            try
            {
                object boxed = scene;
                Type sceneType = boxed.GetType();

                PropertyInfo handleProperty = AccessTools.Property(sceneType, "handle");
                if (handleProperty != null)
                {
                    object value = handleProperty.GetValue(boxed, null);
                    int coerced = CoerceSceneHandleToInt(value);
                    if (coerced != -1)
                        return coerced;
                }

                FieldInfo handleField = AccessTools.Field(sceneType, "handle") ?? AccessTools.Field(sceneType, "m_Handle");
                if (handleField != null)
                {
                    object value = handleField.GetValue(boxed);
                    int coerced = CoerceSceneHandleToInt(value);
                    if (coerced != -1)
                        return coerced;
                }
            ...
            {
                Universe.LogWarning($"Failed to resolve Scene.handle compatibly: {ex.Message}");
            }

            return -1;
        }

        private static int CoerceSceneHandleToInt(object value)
        {
            if (value == null)
                return -1;
            ...
            try
            {
                ...
                foreach (string methodName in new[] { "op_Implicit", "op_Explicit" })
                {
                    MethodInfo method = valueType
                        ...
                        .FirstOrDefault(m => m.Name == methodName
                            ...
                            && (m.ReturnType == typeof(int) || m.ReturnType == typeof(uint)));
                    ...
                    if (method != null)
                    {
                        ...
                        if (converted is int convertedInt)
                            return convertedInt;
                        if (converted is uint convertedUInt)
                            return unchecked((int)convertedUInt);
                    }
                }

                foreach (string memberName in new[] { "m_Handle", "handle", "m_Value", "value", "m_Id", "id" })
                {
                    FieldInfo field = AccessTools.Field(valueType, memberName);
                    if (field != null)
                    {
                        int coerced = CoerceSceneHandleToInt(field.GetValue(value));
                        if (coerced != -1)
                            return coerced;
                    }

                    PropertyInfo property = AccessTools.Property(valueType, memberName);
                    if (property != null && property.GetIndexParameters().Length == 0)
                    {
                        int coerced = CoerceSceneHandleToInt(property.GetValue(value, null));
                        if (coerced != -1)
                            return coerced;
                    }
                }
