...
namespace UniverseLib.Input
{
    public static class EventSystemHelper
    {
        ...
        static float timeOfLastDuplicateEventSystemLog;
        ...
        static void InitPatches()
        {
            ...
            Universe.Patch(typeof(EventSystem),
                "OnEnable",
                MethodType.Normal,
                postfix: AccessTools.Method(typeof(EventSystemHelper), nameof(Postfix_EventSystem_OnEnable)));
        }


        internal static void Postfix_EventSystem_OnEnable(EventSystem __instance)
        {
            try
            {
                if (!__instance || !UniversalUI.EventSys || ConfigManager.Disable_EventSystem_Override)
                    return;

                if (__instance.ReferenceEqual(UniversalUI.EventSys))
                    return;

                if (!CursorUnlocker.ShouldUnlock && !UniversalUI.AnyUIShowing)
                    return;

                lastEventSystem = __instance;
                lastInputModule = __instance.currentInputModule;

                if (__instance.enabled)
                    __instance.enabled = false;

                if (Time.realtimeSinceStartup - timeOfLastDuplicateEventSystemLog > 2f)
                {
                    timeOfLastDuplicateEventSystemLog = Time.realtimeSinceStartup;
                    Universe.LogWarning($"Disabled duplicate EventSystem '{__instance.name}' while UniverseLib UI is active.");
                }
            }
            catch { }
        }
