...
namespace UnityExplorer.UI.Panels
{
    public abstract class UEPanel : PanelBase
    {
        ...
        public string LocalizedName => LocalizationManager.GetPanelLabel(PanelType, Name);
        ...
        public override void SetActive(bool active)
        {
            ...
            else if (RegisteredWithOwner && this.UIRoot && this.UIRoot.transform)
            {
        ...
        public override void ConstructUI()
        {
            ...
            if (NavButtonWanted)
            {
                if (!UIManager.TryConsumeDeferredNavButton(PanelType, out ButtonRef deferredButton))
                    deferredButton = UIFactory.CreateButton(UIManager.NavbarTabButtonHolder, $"Button_{PanelType}", LocalizedName);

                NavButton = deferredButton;
                UIManager.ConfigureNavButton(NavButton, LocalizedName);
        ...
        public virtual void RefreshLocalizedText()
        {
            try
            {
                if (TitleBar != null)
                {
                    Transform titleTf = TitleBar.transform.Find("TitleBar");
                    if (titleTf != null)
                    {
                        Text titleText = titleTf.GetComponent<Text>();
                        if (titleText != null)
                            titleText.text = LocalizedName;
                    }
                }

                if (NavButton != null && NavButton.ButtonText != null)
                    NavButton.ButtonText.text = LocalizedName;
            }
            catch { }
        }
