...
namespace UnityExplorer.Inspectors
{
    ...
    public class ReflectionInspector : InspectorBase, ICellPoolDataSource<CacheMemberCell>, ICacheObjectController
    {
        ...
        readonly Dictionary<Toggle, MemberFilter> memberTypeToggleFilters = new();
        readonly Dictionary<Toggle, bool> memberTypeToggleStates = new();
        ...
        private void SetTarget(object target)
        {
            ...
            {
                toggle.isOn = true;
                memberTypeToggleStates[toggle] = true;
            }
        ...
        void PollMemberTypeToggleChanges()
        {
            foreach (Toggle toggle in memberTypeToggles)
            {
                if (!memberTypeToggleStates.TryGetValue(toggle, out bool lastValue))
                {
                    memberTypeToggleStates[toggle] = toggle.isOn;
                    continue;
                }

                if (toggle.isOn == lastValue)
                    continue;

                memberTypeToggleStates[toggle] = toggle.isOn;

                if (memberTypeToggleFilters.TryGetValue(toggle, out MemberFilter flag))
                    OnMemberTypeToggled(flag, toggle.isOn);
            }
        }
        ...
        void AddMemberTypeToggle(GameObject parent, MemberTypes type, int width)
        {
            ...
            memberTypeToggleFilters[toggle] = flag;
            memberTypeToggleStates[toggle] = toggle.isOn;
        }
    }
}
